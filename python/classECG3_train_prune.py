...
SV_params_V = {'kernel':'rbf','C':0.1, 'gamma':0.1, 'degree':3, 'FS':None, 'pruning_D': 0, 'class_weight':'balanced','type':'VvX'}
SV_params_S = {'kernel':'rbf','C':0.1, 'gamma':0.1, 'degree':3, 'FS':None, 'pruning_D': 0, 'class_weight':'balanced','type':'NvS'}
...
FS_V = FE_preselect_V[[32, 30, 16, 34, 21, 31, 35, 23,  6, 27]]
FS_S = FE_preselect_S[[20,  5,  4, 25, 16, 26, 21, 24,  6,]]
print(FS_V)
print(FS_S)

SV_params = {'kernel':'rbf','C':0.1, 'gamma':0.1, 'degree':3, 'FS':FS_V, 'gamma_2':0.1, 'FS_2':FS_S, 'pruning_D': 0, 'class_weight':'balanced','type':'hier'}

scaler = StandardScaler()
features_train=scaler.fit_transform(features_train)
features_test=scaler.transform(features_test)

j_val = ECGclassification.cross_val(features_train, subset_train, labels_train, SV_params)
cm_test = ECGclassification.train_and_test(features_train, labels_train, features_test, labels_test, SV_params)
j_test = ECGClassEval.evalStats_3(cm_test)

j_val_prune = ECGclassification.cross_val(features_train, subset_train, labels_train, SV_params, prune=True)
cm_test = ECGclassification.train_and_test(features_train, labels_train, features_test, labels_test, SV_params, prune=True, verbose=True)
j_test_prune = ECGClassEval.evalStats_3(cm_test)
