...
int extract_features_RR(beat_t* beat){
  ...
  beat->features->ratio1_RR = (beat->features->prev_RR<<FEATURES_RR_RATIO_SHIFT)/(beat->features->prev2_RR+1);
  beat->features->ratio2_RR = (beat->features->next_RR<<FEATURES_RR_RATIO_SHIFT)/(beat->features->prev_RR+1);
...
int delete_features(features_t* features){
  free(features->time);
  free(features->dwt);
  free(features->dwt_diff);
  free(features);
  return 0;
}

int close_features_buffer(){
